...
export default function ShopRates() {
  ...
  const loadShops = useCallback(async () => {
    try {
      ...
      if (shopsData.length > 0) {
        ...
        Alert.alert("No Shops Found", "You need to register a shop first.", [
          {
            text: "Register Shop",
      ...
      handleError(error, "ShopRates.loadShops", "Failed to load shops");
  ...
  const loadRates = async (shopId: number) => {
    try {
      ...
      handleError(error, "ShopRates.loadRates", "Failed to load rates");
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      handleError(error, "ShopRates.handleSubmit", "Failed to save rate");
  ...
  return (
    <SafeAreaView
      ...
      <ScrollView style={{ flex: 1 }} contentContainerStyle={{ padding: 16 }}>
        ...
        {/**TODO: Add button to go to edit view */}
        <Text
          style={{
            color: "#fff",
            fontSize: 24,
